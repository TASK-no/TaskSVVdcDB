...
mod_cat_choices_A_ui <- function(id) {
  ...
  list_choices <- list(
    Q36 = c(
      "Ja, kjenner litt, noe og godt til",
  ...
  list_cat_ui <- lapply(
    list_QXX, generate_cat_ui, list_headers,
...
mod_cat_choices_data_srv <- function(id, data_seg) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(
      {
        list(
          tmp1 = gargoyle::watch("data_seg"),
          tmp2 = input[["action_cat_choices"]]
        )
      },
      {
        list_recodes_taken <- get_list_cat_choices(
          input[["cat_Q36"]],
          input[["cat_Q37"]],
          input[["cat_Q38"]],
          input[["cat_Q40"]]
...
get_list_cat_choices <- function(input_q36, input_q37, input_q38, input_q40) {
  ...
  list_cat_choices$Q36 <- update_cat_choices(
    input_q36_taken,
    list_cat_choices$Q36
  )
  list_cat_choices$Q37 <- update_cat_choices(
    input_q37_taken,
    list_cat_choices$Q37
  )
  list_cat_choices$Q38 <- update_cat_choices(
    input_q38_taken,
    list_cat_choices$Q38
  )
  list_cat_choices$Q40 <- update_cat_choices(
    input_q40_taken,
